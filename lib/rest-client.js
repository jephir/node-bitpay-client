...
if (HOME && fs.existsSync(HOME + "/.bitpay/config.json")) {
	defaultConf = require(HOME + "/.bitpay/config.json")
}
...
RESTClient.prototype._sendRequest = function() {
	...
	var timeout = self.options.config.timeout || 5000
	var options = { method: method, url: url, strictSSL: false, timeout: timeout }
	...
	if (self.options.signRequests && this.facade !== "public") {
		options.headers["x-identity"] = bitauth.getPublicKeyFromPrivateKey(
			this.privkey
		)
		options.headers["x-signature"] = bitauth.sign(
			this._dataToSign,
			this.privkey
		)
	}
	...
	function handleResponse(err, res, body) {
		...
		try {
			...
			return callback(
				new Error(
					"Server did not return JSON and gave statusCode " + res.statusCode
				)
			)
		}
...
RESTClient.prototype._resourceArgs = function() {
	...
	var data =
		typeof args[1] === "function" ? (noPath ? args[0] : {}) : args[1] || {}
	var lastArg = args[args.length - 1]
	var callback = typeof lastArg === "function" ? lastArg : new Function()
